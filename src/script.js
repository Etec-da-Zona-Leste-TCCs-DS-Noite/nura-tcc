    // --- INITIALIZE NURANOTIFY GLOBAL TOAST SYSTEM ---
    window.NuraNotify = window.NuraNotify || {
        toast: function (msg, type = 'success') {
            const existing = document.querySelector('.nura-toast-host');
            if (existing) {
                existing.remove();
            }

            const toast = document.createElement('div');
            toast.className = `nura-toast-host nura-toast-host--${type}`;

            let iconHtml = '';
            if (type === 'success') {
                iconHtml = '<i class="ph-fill ph-check-circle nura-toast-host__icon"></i>';
            } else if (type === 'error') {
                iconHtml = '<i class="ph-fill ph-x-circle nura-toast-host__icon"></i>';
            } else {
                iconHtml = '<i class="ph-fill ph-info nura-toast-host__icon"></i>';
            }

            toast.innerHTML = `
                ${iconHtml}
                <span class="nura-toast-host__text">${msg}</span>
            `;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Auto-switch to Signup tab if registration failed
        try {
            const params = new URLSearchParams(window.location.search);
            const flash = params.get('nura_flash');
            if (flash && (flash.toLowerCase().includes('cadastro') || flash.toLowerCase().includes('cadastrar'))) {
        ...
        carousels.forEach(container => {
            ...
            container.addEventListener('mouseenter', stopPlay => stopAutoPlay());
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const localSignupPassword = document.getElementById('local-signup-senha');
        const localStrengthFill = document.getElementById('local-strength-fill');
        const localStrengthText = document.getElementById('local-strength-text');

        function checkPasswordStrength(inputEl, fillEl, textEl) {
            if (!inputEl || !fillEl || !textEl) return;
            inputEl.addEventListener('input', function (e) {
                ...
                if (val.length === 0) {
                    ...
                    textEl.textContent = 'Mínimo 8 caracteres';
                    textEl.style.color = 'var(--muted)';
                } else if (strength < 50) {
                    fillEl.style.background = 'var(--danger)';
                    textEl.textContent = 'Senha fraca';
                    textEl.style.color = 'var(--danger)';
                } else if (strength < 100) {
                    fillEl.style.background = 'var(--warning)';
                    textEl.textContent = 'Senha média';
                    textEl.style.color = 'var(--warning)';
                } else {
                    fillEl.style.background = 'var(--green-leaf)';
                    textEl.textContent = 'Senha forte!';
                    textEl.style.color = 'var(--green-leaf)';
                }
            });
        }

        checkPasswordStrength(signupPassword, strengthFill, strengthText);
        checkPasswordStrength(localSignupPassword, localStrengthFill, localStrengthText);
    });
